"""Physical qualification for the bounded operator presentation controller."""
...
from k5vision.media.presentation_controller_evidence import (
    PresentationControllerPhysicalEvidence,
    write_evidence,
)
...
pytestmark = pytest.mark.skipif(
    os.getenv("K5_STAGE31_PHYSICAL") != "1",
    reason="Stage 31 physical qualification is opt-in",
)
...
def _physical_context() -> tuple[str, str, Path, str, Path]:
    ...
    output = Path(os.environ["K5_STAGE31_OUTPUT"])
    revision = os.environ["K5_STAGE31_REVISION"].casefold()
...
def test_stage31_controller_runs_mixed_live_playback_to_viewports() -> None:
    source, authenticated_source, output, revision, runner_temp = _physical_context()
    scratch = runner_temp / f"k5-stage31-{uuid4().hex}"

    async def qualify() -> PresentationControllerPhysicalEvidence:
        recording_id = uuid4()
        source_id = uuid4()
        ...
        try:
            ...
            controller = BoundedPresentationController(session, stop_timeout_seconds=5.0)
            streams = [
                MixedLiveStream(0, authenticated_source, live_delivery),
                MixedPlaybackStream(1, playback_delivery),
            ]
            started_snapshot = await controller.start(streams)
            assert started_snapshot.state == PresentationControllerState.RUNNING
            snapshot = await controller.wait()

            assert snapshot.state == PresentationControllerState.COMPLETE
            ...
            return PresentationControllerPhysicalEvidence(
                revision=revision,
