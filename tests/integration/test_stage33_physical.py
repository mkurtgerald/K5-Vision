"""Physical qualification for bounded re-entry above the presentation runtime."""
...
pytestmark = pytest.mark.skipif(
    os.getenv("K5_STAGE33_PHYSICAL") != "1",
    reason="Stage 33 physical qualification is opt-in",
)
...
def _physical_context() -> tuple[str, str, Path, str, Path]:
    ...
    output = Path(os.environ["K5_STAGE33_OUTPUT"])
    revision = os.environ["K5_STAGE33_REVISION"].casefold()
...
def test_stage33_host_reenters_fresh_mixed_runtime_generation() -> None:
    source, authenticated_source, output, revision, runner_temp = _physical_context()
    scratch = runner_temp / f"k5-stage33-{uuid4().hex}"

    async def qualify() -> PresentationHostPhysicalEvidence:
        recording_id = uuid4()
        source_id = uuid4()
        ...
        try:
            ...
            for generation in (1, 2):
                started_snapshot = await host.start(streams())
                assert started_snapshot.state == PresentationHostState.RUNNING
                assert started_snapshot.generation == generation
                completed_snapshot = await host.wait()
                assert completed_snapshot.state == PresentationHostState.COMPLETE
                assert completed_snapshot.completed_generations == generation

            done = host.snapshot
            assert done.generation == 2
            assert done.completed_generations == 2
            assert done.stopped_generations == 0
            ...
            assert frames_by_slot[0] >= 2
            assert frames_by_slot[1] >= 2
            ...
            return PresentationHostPhysicalEvidence(
                revision=revision,
                execution_context="camera-lab-windows-x64",
                generation_limit=closed.generation_limit,
                generations_started=closed.generation,
