"""Physical qualification for bounded concurrent playback presentation delivery."""
...
from k5vision.media.multiview_playback_presentation_evidence import (
    MultiViewPlaybackPhysicalEvidence,
    write_evidence,
)
...
pytestmark = pytest.mark.skipif(
    os.getenv("K5_STAGE27_PHYSICAL") != "1",
    reason="Stage 27 physical qualification is opt-in",
)
...
def _physical_context() -> tuple[str, str, Path, str, Path]:
    ...
    output = Path(os.environ["K5_STAGE27_OUTPUT"])
    revision = os.environ["K5_STAGE27_REVISION"].casefold()
...
def test_stage27_two_concurrent_playbacks_reach_serialized_consumer() -> None:
    source, authenticated_source, output, revision, runner_temp = _physical_context()
    scratch = runner_temp / f"k5-stage27-{uuid4().hex}"

    async def qualify() -> MultiViewPlaybackPhysicalEvidence:
        recording_id = uuid4()
        source_id = uuid4()
        ...
        live_delivery = EphemeralRtpDelivery(
            packet_goal=2048,
        ...
        try:
            await live_delivery.deliver(authenticated_source, record)
            await sink.finalize()
            ...
            return MultiViewPlaybackPhysicalEvidence(
                revision=revision,
