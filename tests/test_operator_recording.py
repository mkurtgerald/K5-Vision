...
class _StaticResolver:
    ...
    async def resolve(self, device, stream_token: str) -> ResolvedLiveSource:
...
def _registry(tmp_path: Path) -> tuple[DeviceRegistry, object]:
...
@pytest.mark.asyncio
async def test_operator_recording_persists_replay_ready_framed_recording(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_operator_recording_rejects_viewer_without_creating_media(tmp_path: Path) -> None:
    registry, device = _registry(tmp_path)
...
@pytest.mark.asyncio
async def test_operator_recording_rejects_source_outside_enrolled_device_scope(
    tmp_path: Path,
) -> None:
    registry, device = _registry(tmp_path)
    root = tmp_path / "recordings"
