...
@pytest.mark.asyncio
async def test_finalize_promotes_only_complete_payload(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_abort_removes_partial_payload(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_existing_final_recording_is_never_overwritten(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_same_id_partial_writer_conflicts(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_sink_enforces_defense_in_depth_byte_limit(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_invalid_state_transitions_are_deterministic(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_stage07_recorder_integrates_with_local_sink(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_recorder_abort_deletes_local_partial_payload(tmp_path: Path) -> None:
